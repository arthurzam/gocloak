...
func Test_DeleteUserBruteForceDetectionStatus(t *testing.T) {
	cfg := GetConfig(t)
	client := NewClientWithDebug(t)
	token := GetAdminToken(t, client)
	realm, err := client.GetRealm(
	...
	err = client.DeleteUserBruteForceDetectionStatus(
		context.Background(),
		token.AccessToken,
		cfg.GoCloak.Realm,
		userID)
	require.NoError(t, err, "Deleting attack status failed")
